...
# Load libraries
library(signal)    # For digital filtering
library(tuneR)     # For audio signal processing
library(seewave)   # For spectral analysis
require(zoo)
...
df_list <- readRDS(file = "data/df_list_sample3.rds")

#get signal
data<-df_list[[4]]
data_firstblock<-data[data$block==3,]
signal<-data_firstblock[['left_pupil_measure1']]
timestamp<-data_firstblock[['logged_time']]
names(data_firstblock)
...
# use only content of pupil_preprocessing
# df_preprocessed<-PupilPreprocess::pupil_preprocessing(data,
# provide_variable_names = T, left_diameter_name = "left_pupil_measure1", right_diameter_name = "right_pupil_measure1", 
# timestamp_name = "logged_time",timestamp_scaling=0.001)
...
for (i in seq_along(chunks_normalized)) {
  hist(chunks_normalized[[i]], xlim = c(-3, 3), breaks=20, main = paste0("Chunk ", i, " values"))
}
par(mfrow=c(1,1))
